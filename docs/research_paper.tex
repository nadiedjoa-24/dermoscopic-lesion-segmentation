...
A coverage test guards the whole stage: if the eroded hair response covers less
than 20\,\% of the valid area, the image is returned untouched. Inpainting a
hairless image only blurs the very border the segmentation is looking for.
...
$0.35$ Dice on a single image (ISIC\_0000146: $0.440$ before correction, $0.792$
...
the lesion. The largest weighs $23\,\%$ of the main region's own area and sits
over $1100$\,px away, so the old rule glued it, and several smaller ones, into
the hull regardless of distance: the buggy hull scores just $0.184$ Dice on
this image, and the fix (distance alone) recovers it to $0.620$. Being large
...
raises the mean Dice of Otsu, LBP and SRM alike by roughly $0.041$–$0.057$,
