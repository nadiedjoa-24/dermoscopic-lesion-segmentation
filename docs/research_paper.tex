...
\emph{every one} of the twenty images, from $3.5\,\%$ more on the least hairy
images to over $30\,\%$ more on some of the hairiest. Diagonal hairs were
...
The coverage test above (Section~1.2.2) is meant to measure a pixel fraction:
...
costs the region-merging method $0.124$ Dice ($0.724$ left alone, $0.600$
...
Applying the method as published to our cropped images failed completely on two
of them, with a Dice score of exactly zero. The cause is the interaction
announced in Section~1.1: frame removal whitens the corners, and a white pixel
...
Restricting the clustering to the valid disc area resolves it, and the two
failing images rose from $0.000$ to $0.915$ and $0.957$. The lesson generalises
...
\begin{itemize}
  ...
  percentile of a band along the image border, following the observation of
  Zortea et al.~\cite{zortea} that this band is almost entirely healthy skin
  after cropping. The blue channel is used, for the melanin-absorption reason
...
Section~2.2.4 already covers why LBP's clustering has to ignore the whitened
frame corners left by frame removal (Section~1.1): unguarded, a global colour
...
skin reference is read off a one-pixel band along the image border, and a
circular disc inscribed in a rectangular crop still grazes a sliver of frame
in each corner.
...
source of bias, for a Dice change of only $0.673 \rightarrow 0.667$ on this
image: the fix is about correctness, not chasing a dramatic number here.
...
higher: Dice on this image falls from $0.724$ to $0.510$. The reason is that
...
satellite component only if it is large relative to the main one (more than
3\,\% of its area) or close to it. Distant specks of noise are discarded, so
the convex shape stays meaningful.
...
The dermoscope disc is known exactly: frame removal (Section~1.1) already
...
array instead. A disc inscribed in a non-square rectangle covers only about
half of it: measured across this dataset, a mask that fills the disc
completely reads as $51$–$57\,\%$ of the array, depending on its aspect ratio
— under the $60\,\%$ threshold on every single image. The guard was not
merely miscalibrated; it was, in practice, unreachable. The fix compares
...
dataset the farthest corner sits at $51$–$57\,\%$ of the diagonal regardless of
...
sits. Fixing the previous three guards let raw masks reach this code path for
the first time on unframed images, and exposed why that reasoning fails in
...
inspection that found the channel-mixing bug in Section~2.3.2.
...
raises the mean Dice of Otsu, LBP and SRM alike by roughly $0.046$–$0.056$,
...
All three methods score lower on melanomas than on nevi. This is the clinically
expected direction rather than a defect: irregular, poorly defined and
asymmetric borders are precisely what characterises melanoma, and precisely what
the ABCD rule looks for. A segmentation method that performed equally well on
both would be suspicious.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{dice_summary.pdf}
  \caption{Mean Dice per method with min--max whiskers, melanoma (left) and
  nevus (right). The spread is markedly wider on melanomas.}
  \label{fig:summary}
\end{figure}
